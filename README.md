...
- **想自己手算帧号**：见 [HAND-COMPUTE.md](HAND-COMPUTE.md)（完整口径 + 四个可对照样例 + 证据表）。
- 敌人 prefab 的 `_delayToBorn`（客户端 `Scheduler::_DealAction` 的 `max(t - v, 0)`）也算在内：2105 个 prefab 里 164 个非零，**1049 个关卡**的生成帧因此比不带该修正时早若干帧；0-x/1-x 全 0 不受影响。变体 id（`enemy_2133_shdopl_b`）会按 base id（`enemy_2133_shdopl`）查同一字段。
- 帧数由客户端配置 + 复原出来的调度算术算出（`preDelay` 累加、同刻 Mono 快排、每条目至少 1 帧、fragment 边界 2 帧）。
- `wave.preDelay > 0` 的波次里，「合成条目 → 第一条真实条目」那一跳少记 1 帧（六条实测帧钉死：0-1 62/154/155、9-11 预览 180/190 + 首怪 271/512、CE-5 0/1/2）。
